...
function SignupContent() {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        localStorage.setItem(
          "dynoquizz_user",
          JSON.stringify({ email: email.trim(), role: returnedRole, name: name.trim() }),
        );
      }
      const redirectTarget = searchParams?.get("redirect");
      const destination = returnedRole === "TEACHER" ? "/dashboard/teacher" : "/dashboard/student";
  ...
  // ─── 2. MINIMAL SINGLE-COLUMN SIGNUP FORM ──────────────────────────────────
  return (
      ...
      <div className="w-full max-w-sm space-y-6">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-neutral-900">
            {activeRole === "teacher" ? "Sign up as Instructor" : "Sign up as Student"}
          </h1>
        </div>
